...
def determine_mane_version(ensembl_mane_lookup_path, ensembl_release):
...
def main(paths, config):
    ...
    config_path, ensembl_mane_lookup_path, precompiled_conservation_path, precompiled_shape_path = paths
    settings, directories, cores = config

    print("\n00/11 Downloading annotation data...")
    mane_version = determine_mane_version(
        ensembl_mane_lookup_path, settings["ensembl_release"])
    run_subprocess(
        ["sh", "./src/download_annotation_data.sh", settings["use_caching"],
            directories["preload_data"], settings["ensembl_release"], mane_version],
        "An error occurred while downloading annotation data."
    )
    print("00/11 Complete.\n")

    print("01/11 Parsing annotation data and extracting 3' UTR / CDS sequences...")
    run_subprocess(["Rscript", "src/parse_annotation_data.r", config_path],
                   "An error occurred while parsing annotation data.")
    ...
    run_subprocess(["Rscript", "src/generate_conservation_scores.r", config_path, cores],
                   "An error occurred while generating conservation scores.")
    print("02/11 Complete.\n")

    print("03/11 Locating binding sites for each miRNA...")
    run_subprocess(["Rscript", "src/locate_binding_sites.r", config_path, cores],
                   "An error occurred while locating binding sites.")
    print("03/11 Complete.\n")

    print("04/11 Extracting folding windows for each miRNA...")
    run_subprocess(["Rscript", "src/extract_windows.r", config_path, cores],
                   "An error occurred while extracting folding windows.")
    ...
    run_subprocess(["Rscript", "src/extract_features.r", config_path, cores],
                   "An error occurred while extracting features.")
    ...
    if literal_eval(settings["use_precompiled_shape"]):
        print("Using precompiled data...")
        with tarfile.open(precompiled_shape_path, "r:gz") as tar:
            tar.extractall(path=".")
    else:
        print("Using fresh data...")
    ...
    run_subprocess(["Rscript", "src/impute_missing_values.r",
                   config_path, cores], "An error occurred while imputing values.")
    print("10/11 Complete.\n")

    print("11/11 Making predictions using machine learning model...")
    MachineLearning(settings, directories, cores)
...
if __name__ == "__main__":
    ...
    print("Starting miRsight...")

    main((CONFIG_PATH, ENSEMBL_MANE_LOOKUP_PATH,
         PRECOMPILED_CONSERVATION_PATH, PRECOMPILED_SHAPE_PATH), load_config(CONFIG_PATH))
